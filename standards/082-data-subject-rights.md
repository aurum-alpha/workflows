...
kind (`relational`, `document`, `blob`, `external`), and the column that ties
a row to the subject. Where the subject is reached indirectly, the entry names
...
recipient, and an audit event with an actor and a target. Erasure targets a
subject, and a subject can appear in either position. An implementation that
reasons per row gets exactly one of the two right. That is the failure 080
AE7's redaction corpus already detects, arriving here for every table.
...
has an entry. The coverage check enumerates the catalog to prove it. That is
025 SD6's pattern, for the same reason. A list of tables to check rots the day
someone adds a table, and an enumeration cannot. So absence from the inventory
is the finding, rather than a table quietly assumed clean.

The check also refuses an inventory it cannot execute. That is a retained or
excluded column the table lacks, a `via` naming no entry, or two membership
entries (DR4). It is also a `NOT NULL` non-text column that is neither
structural nor retained, for which reduction has no value to write. That last
case is fixed by an expand migration (025 SD4) or by retaining the column. It
is never fixed by the job improvising a sentinel.

### DR2. Both rights are request resources under one status machine

A requester exercises either right by creating a request resource, shaped by
...
finds every export of one person's data, whoever asked (080 AE5's sixth
floor).

**Statuses** are one vocabulary for both kinds: `pending`, `running`,
`completed` and `failed`. Per kind there are `expired` (an export whose
package window closed), and `cancelled` and `held` (an erasure). The schema
binds which fields each status requires. A completed export requires its
package, and a completed erasure its result (DR7). A held erasure requires its
...
**One open request per subject per kind**. A second creation while one is open
returns the open resource with `200`, rather than a new one with `201`.
...
hours after the grace period for an erasure. The strictest common regime
allows a month. The 72-hour figure is an engineering floor. Once the inventory
exists the work is a job over a declaration, and only a backlog makes it slow.
A request open past its deadline is an alert, computed from the request table
the way 057 JB8 computes staleness.
...
The pool then runs `export.build`: per-event, serial per request, `long`,
`idempotent`. It is never "process pending exports nightly", which 057
refuses.
...
lists every file with its SHA-256. It names the request, subject, tenant, the
...
material (a session, whose id is a credential under 090 WC1). Or they are a
derived projection of entries already exported (a search index under 027). Or
they are a provider's copy of what the service already exports.
`export_excludes` names the columns withheld from an exported entry: a
credential hash, or the columns describing another person in a shared row.
**The package never carries a credential, a secret, an internal key, or
...
receives two packages from two requests. That is 025 SD6's isolation applied
to the one read that crosses every table. **Delivery is through the service,
and never a URL, on the resource or anywhere else.**

The job writes the package to the service's own bucket under 026, as an object
the request row owns. `GET …/{id}/download` checks the requester and streams
the object per [`026-blob-storage.md`](026-blob-storage.md) BS5 with
`Content-Disposition: attachment`, answering `200`. The package is available
for seven days from completion. After that the route answers `410
...
**Creating an erasure request starts a grace period, fourteen days by default,
during which nothing moves and the subject can cancel**. The subject is told
at creation and at cancellation through 058's security floor. So a request
made from a borrowed session is seen by its owner while it can still be
withdrawn. A product shortens the grace in its **Conventions** with a reason.
It never lengthens the grace past the point where grace plus the 72-hour floor
exceeds the shortest statutory deadline it is under. Fourteen days is the
longest default that leaves room for a failed run and a retry under a
one-month regime.

Once grace has ended the request is no longer cancellable (`409
erasure-not-cancellable`). The data is moving in batches, and a cancellation
now would leave a state nobody declared.
...
hourly, single-flight. It selects requests whose grace has ended and that
carry no hold (DR6), and moves each to `running`. It writes one `erasure.due`
message per request to the outbox. `subject.erase` runs per event, serial per
subject, `long`, `idempotent`, and does the work.
...
Within an entry the work is keyset batches, one transaction each, with a
checkpoint after every batch (057 JB7, JB10). The job applies the declared
treatment (DR5) to the tenant's rows for a scoped entry, and to all rows for a
global one.

Blobs go through 026's mechanism as their owning rows go, and documents
through 027's. An `external` entry goes through the provider's interface, and
a failure there fails the run. Notification suppression under 058 happens
first.
...
**The order is fixed, and the corpus checks it**. It is suppression, scoped
entries children-first, the membership test, global entries children-first,
`revokeAppAccess`, the two records of DR7, then `completed`.
...
answers. There are three things a row can be after its subject leaves: gone,
kept without the person, or kept with the person under an obligation.
...
**Structural columns survive every reduction**. They are the internal key, the
public id, the isolation columns (025 SD5), `created_at` and `updated_at`, the
entry's subject column, and the stamp columns. **Everything else is tombstoned
unless the allowlist names it**: `NULL` where nullable, the literal `[erased]`
where `NOT NULL` text.
...
missing ceiling. In every treatment the job reads the declaration and applies
the tombstone rule. A product wanting a richer transformation, such as a
generalised postcode or a bucketed age, declares the derived column as
retained. It populates that column in ordinary code. So the job stays a
function of the declaration and is never ad hoc.
...
holding `legal_hold.place` and `legal_hold.release`**. **Both acts are
...
does not suspend export: it preserves evidence, not the person's right to see
it. Nor does it suspend cancellation, which remains the subject's while
nothing has run.
...
administrator's reason is not, because it can name a dispute the subject is a
...
writing two records in the same transaction as its last batch. A request is
`completed` only when both exist. **The audit event** (080) has action
`data_subject.erase`, actor the requester, and target the subject. The job
writes it *after* the subject's identification is gone. So it carries the
subject's public id and no display, with `erased_at` and `erased_subjects`
naming the subject from the moment it is written.

The event that proves the erasure is the first event that conforms to it.
**The erasure ledger entry** (028) carries the subject's public id, the
tenant, the request id, the entries treated and the instant. It is what a
restore replays before a service is readmitted to traffic, so a backup taken
before the erasure cannot resurrect what it removed. Its retention is 028's to
state. The ledger's treatment vocabulary is 028's: `delete`, `anonymise`, and
`redact` for an append-only store carrying AE7's stamp. `redact` is what this
standard's `anonymise` means on the audit store.

A `retain` entry writes nothing to the ledger, because nothing was done to the
row. The longest any copy of an erased subject's data persists is the
service's `erasure_horizon` (028 BR5). That is the number stated to subjects.
The request's `result` carries both ids, the counts, and whether identity
access was revoked. So the resource answers *what happened* without a log
query.

### DR8. A tenant leaving takes its subjects' data with it

A tenant offboarding is an erasure over every row the tenant owns. It runs as
an operator-triggered job, `tenant.offboard`: on-demand, single-flight,
`long`, `idempotent`. Its shape depends on 025 SD6's isolation mechanism, and
is stated here so the mechanism is chosen with this cost known. Under **one
database per tenant** it is a reduction followed by a drop. The retain and
anonymise treatments run, so that only rows with a legal basis remain. The
database is held read-only under the retention credential until
`retention.purge` reports nothing left, and the drop is the final operator
step.

The bucket follows 026's deletion rules the same way. Under **row-level
isolation** it is an erasure over every scoped table, enumerated from the
catalog as SD6's gate enumerates them, children before parents. In both,
subjects who belong to other tenants keep their rows there. Global rows go
only when DR4's membership test says so. The tenant's *subjects'* data is
treated by the same declaration as any single erasure.
