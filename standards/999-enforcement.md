...
| AU5 | Sessions end, and revocation does not wait for them to | live gates are available for the observable half: a session refusing service after its absolute cap, and logout ending the proxy session (proposed). Ending the provider session too is a product option, not the default | **review only** |
| AU6 | An authenticated subject the application does not know is refused, and logged out | **a clean live behaviour case**: authenticate as an unknown subject, require 403 with the session ended. The `entitlements` block and `user.editable_fields` are schema-decided by `me.schema.json`. That the server also enforces every permission and entitlement resists a checker and is the review question | **review only** |
...
| — | The auth corpus decides the token and the client identity document | `job-contract-conformance` running [`../contracts/auth/corpus.json`](../contracts/auth/corpus.json) (proposed). **The two halves run in different places and the row states both.** The `validity` half is static: each case validates against `me.schema.json` and reaches its stated verdict, with nothing running. The token has no validity case, because this repository describes no shape for it. The `behaviour` half needs a live stack with a provider behind it, so it runs where a repository has one and decides nothing where it does not | **review only** |
...
| — | One corpus judges every implementation of `check` | `job-contract-conformance` running [`../contracts/rbac/decisions.json`](../contracts/rbac/decisions.json) (proposed). Static, and it needs no service: the fixture, an active grant per case, and the decision each case expects. **It decides the one thing nothing else can, that an implementation in one language and an implementation in another return the same decision**, because both read this file rather than each other | **review only** |
## Audit standard
