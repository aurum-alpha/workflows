...
**Two lanes, because only one of them is worth blocking.**
`production: true` runs `knip --production`, reporting unused `dependencies`
alone: a runtime package nothing reaches ships in the image for nothing, which
is a defect rather than debt. The full lane also reports `devDependencies`,
where the finding is real but cheaper, and runs `warn_only` until a repo is
clean.

`--production` is what separates them, and the obvious alternative does not.
**`devDependencies` is not one of knip's issue types**, so `--include
dependencies` still reports them and `--exclude devDependencies` silently
excludes a type that does not exist. The incantation that looks like it splits
the lanes produces a plausible report and filters nothing.
...
- *Packages reachable only from dead files.* knip prunes unreachable files
  ...
  dependency's name, and it is why the production lane cannot block yet: roughly
  twenty `@radix-ui/*` packages sit behind unused shadcn components.
