...
| — | A rendered development stack equals the source it was rendered from | `check-devkit-drift` (proposed): compare the repository's rendered development-stack files against the tagged source the repository records, and report any difference. Same shape as `check-lint-configs` over `.oxlintrc.json` and `.prettierrc.yaml`, and the finding is the same finding: a shared file edited where it landed rather than where it is owned | **review only** |
## Agent standard
