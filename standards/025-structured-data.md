...
library is permitted to execute it, bind its parameters, and map result rows
onto a language type. A library is **not** permitted to generate SQL at
runtime from an object model, a fluent builder, or a chain of method calls.

The argument is not about taste: *a query you cannot paste into a console and
`EXPLAIN` as-is is one nobody will profile*. The SQL is the review surface,
the debugging surface and the performance surface. A builder chain hides the
one thing every one of those needs. Object-relational mapping also abstracts
the engine as though it were swappable. That trades the review surface for a
...
- **Code generation from authored SQL is admitted and encouraged.** `sqlc` is
  the model. A tool of that kind reads the `.sql` file and emits a typed
  function to call it. It keeps the SQL as the source and adds type safety on
  top. The distinction is direction: SQL in, code out is fine; model in, SQL
  out at runtime is not.

A thin row mapper is not an ORM. Scanning a result row into a struct or a
record is the driver's job. That is what "map result rows onto a language
type" means. The line is whether the library *wrote the query*.
...
is `WHERE key > $1 ORDER BY key LIMIT $2` over a key that is unique, indexed
and monotonic. `OFFSET n` fails on both counts that matter. It is O(n),
because the engine walks and discards n rows on every page. It is unstable,
...
**A schema change outside local development is a migration**. It is a file
of SQL, in a single migrations directory, with a name that orders it. It is
committed and reviewed like any other change. Once merged its bytes never
change. A correction is a new migration, exactly as a ledger is corrected by
a new entry.

**How the file is produced is unconstrained**. It can be written by hand. It
can be generated in development by a tool that diffs the desired schema
against the current one. Both are fine, because the rule binds the artifact
and not the authoring. What lands on disk is SQL a reviewer reads. The tool
that helped write it is not part of the contract.
...
that will run. That is SD1's objection arriving one directory over. Each also
ties the migration history to a language runtime, so that a database cannot
be built or inspected without one. The migrations directory contains `*.sql`
and nothing else.
...
deploy path can reach. That means not in a `Dockerfile`, not in a compose
file that starts a persistent database, and not behind a script named
`migrate`. A repository that carries both a migrations directory and a
reachable push command has two answers to one question. The second answer
will be the one used at the wrong moment.

Each migration applies in its own transaction where the engine allows it.
Then a file's statements commit or roll back together, and the version record
is written only on success. A statement that cannot run inside a transaction,
such as `CREATE INDEX CONCURRENTLY`, opts out explicitly in the file, and
says so.

**Every migration converges.** A migration can run against a database where
it has already applied, in whole or in part after an interrupted run. It then
produces the same end state and exits zero. The guarded forms are
`CREATE TABLE IF NOT EXISTS`, `ADD COLUMN IF NOT EXISTS`,
`CREATE INDEX IF NOT EXISTS`, `DROP … IF EXISTS`, and
`INSERT … ON CONFLICT DO NOTHING` for seed rows. Where the engine has no guard
for a statement, the statement is wrapped in a conditional that checks the
...
runner skips what the version record says has applied, and each file
converges if run regardless (SD2). So a second run against a current database
applies nothing and exits zero. A run against a half-migrated one finishes
...
database role cannot alter the schema; the migrate image's can. Because the
...
**A migration is permitted to add. It is not permitted to drop or retype
anything the previous release reads**. Removing a column takes three releases.
Add the new shape and write both; move reads to the new shape; drop the old
one a release later.

This rule is the entire reason rollback works. Redeploying the previous image
against a schema the current release has migrated forward succeeds. That
schema is a *superset* of what the previous release needs. Without it,
*rollback is a data-loss event wearing a tag change*. It is discovered at the
worst possible moment, because rollback is what one reaches for when
something is already wrong.

A migration containing `DROP COLUMN`, `DROP TABLE`, or `ALTER … TYPE` fails
review and the gate unless it carries a marker. The marker is
`-- expand-only-ok: <the release that stopped reading the old shape>`, on the
same line or the line before. It names the release after which the statement
became safe. The marker is a reason, not a permission. A marker with no
release named is a violation with a comment on it.
...
million rows holds a lock for the duration. The deploy step then times out
with the table half-written. A data change that touches many rows is
therefore three artifacts, not one. The first is a migration that adds the
column nullable. The second is a batched job, in bounded batches, resumable
...
single-tenant tool does not have at all. A platform that hosts many practices
has one above it. A standard that named the levels would fit the products it
was written from and force a fiction on the rest. The fiction is an empty
...
scoped to `tenant` carries `tenant_id`, and a table scoped to `client_org`
carries `client_org_id` **and** `tenant_id`. Containment is declared upward,
and the row must be addressable at every level that contains it. One
hierarchy, declared in one place, is read by both the authorization layer and
the storage layer. A product with two hierarchies has two answers to *who is
...
being queried. A predicate that has to join upward to find the tenant is
...
gate proves. Per PC4 it proves it at the boundary, by issuing the query and
...
|---|---|---|
| **Row-level security** | The engine enforces the predicate on every statement, from a context the connection binds per request. Application scoping remains; RLS is the backstop that makes a missed `WHERE` clause a denied row rather than a leak. | Postgres has it; MySQL does not. The per-request binding (`SET LOCAL`) is a discipline the code must make structural. One way is to make the bound transaction the only handle a query method will accept, so an unbound query cannot be written. |
...
| **Application-layer scoping** | Every query carries the isolation predicate, and the check is in the code path. Ownership checks on individual rows are the authorization layer's, per RB5. | No backstop. A missed predicate is a cross-tenant read that no test exercising one tenant at a time will show. That is why the enumeration gate below exists, and why this mechanism leans on it hardest. |

Whichever mechanism, two rules hold across all three:

- **A missing isolation context denies.** A request that resolved no tenant
  sees no scoped rows, by construction. That covers the platform host, an
  unauthenticated path, and a background job with no tenant bound. Under
  row-level security this falls out of the predicate's shape:
  `has_tenant(NULL)` is false, so an unbound connection matches nothing. An
  application-layer implementation achieves it by refusing to run a scoped
  query with a null scope, rather than by omitting the predicate.
- **No bypass role on scoped tables.** The platform operator's predicate never
  appears in a policy on a tenant-scoped table. A platform-admin flag never
  ...
  membership check like any real membership. [`080-audit.md`](080-audit.md)
  AE2 records that impersonation with the impersonator named. This is
  [`070-rbac.md`](070-rbac.md) RB4 applied to the storage layer: the code
...
sets the session time zone to UTC at open. Then the engine's own casts and
`now()` agree with the profile. A product that relies on the server default
is one configuration change from silently shifting every timestamp it stores.
...
**Seed data** is reference rows the application cannot run without: the
system roles [`070-rbac.md`](070-rbac.md) RB3 keeps in code, a currency
table, a default configuration row. Seeds are **idempotent SQL applied through
the same migrate path**. So a database built from migrations alone is a
database the application can start against. A seed that is not idempotent is
a migration that fails the second time it runs.

**Fixture data** is the rows a test or a demo needs. Fixtures live with the
...
this prevents is a demo tenant, a test user with a known password, or a
placeholder email address found in production. Nobody can say when it
arrived.
...
- **The connection string comes from the environment** per
  ...
  [`030-service.md`](030-service.md) SC3. The credential inside it is a
  secret delivered as the [`032-secrets.md`](032-secrets.md) says. Nothing in
  the repository contains one.
- **The runtime role is the least the service needs**. It cannot create or
  alter tables (SD3), and it cannot bypass isolation (SD6). It holds only the
...
- **The connection is encrypted in transit**, and the pool is **bounded and
  ...
  that hangs.
- **SQL parameters never reach a log, span or metric.** The statement text is
  permitted in a log. The values bound to it are not, because they are user
  data, and under SD6 are tenant data. This is the [observability
  standard](040-observability.md)'s rule about what a line is permitted to
...
Code paths multiply. A rule enforced in one of them is a rule enforced in the
paths someone remembered. The paths are the write handler, the import job,
the admin script, the backfill. The database is the one place every write
goes through. So **an invariant of the data is declared in the schema, and
code is the second line, never the only one.**

- **`NOT NULL` is the default; nullable is a decision**. A column admits
  `NULL` because the domain genuinely has an absent case, stated in the
  migration that adds it. It never admits `NULL` because the author did not
  decide. A sentinel standing in for absence (an empty string, a zero, a date
  in 1970) is a `NULL` that lies about itself. Every query that touches it
  has to know the lie.
- **Foreign keys are declared.** A reference the code promises to honour is
  honoured in the paths that remember to. A declared constraint is honoured
  in all of them, including the one that runs at three in the morning. **And
  every foreign-key column is indexed**, because the engine does not do this
  for you. An unindexed foreign key turns every delete of a parent row into a
  scan of the child table. That is a table lock in a trench coat.
- **Uniqueness and domain rules are `UNIQUE` and `CHECK` constraints.** An
  email unique per tenant is `UNIQUE (tenant_id, email)`, not a `SELECT`
  before the `INSERT`, which races. A status that takes four values is a
  `CHECK`, not a comment.
...
- **A JSON column holds what the application does not query by field**. A
  ...
  filters, joins, sorts or indexes on is a column. A `jsonb` column that
...
- **Isolation columns lead the composite index** on every scoped table (SD5).
  ...
  does. An index on `(created_at)` alone makes
  `WHERE tenant_id = $1 ORDER BY created_at` a scan of every tenant's rows.
- **Every table carries `created_at` and `updated_at`**, both `NOT NULL`, both
  instants per IP4, `updated_at` maintained on every write. The write or a
  trigger maintains it, decided once per repository. They are the two
  questions asked of every row on every support call.

**Identifiers are `snake_case`, decided once.** The argument
[`050-http.md`](050-http.md) HA8 made for the wire is sharper in the schema.
An unquoted SQL identifier case-folds *silently*. So `createdAt` becomes
`createdat` in one reference and stays `createdAt` in a quoted one, and they
are two columns until they are the same one.

Table names are plural nouns (`invoices`, `tenants`). A foreign key is the
singular of the referenced table plus `_id` (`tenant_id`), which is the form
SD5 already assumes. None of these three choices is better than its
alternative in any way that matters. That is exactly why each is made here,
...
must agree are one transaction, or they are a bug waiting for the failure
between them. Examples are the change and its audit event, per
[`080-audit.md`](080-audit.md) AE8, and the order and its lines.

**A transaction never spans a network call or a human**. Not an HTTP request
to another service, not a queue publish, not a wait for the user's next
click. A transaction holds locks and a pooled connection for its lifetime.
One that waits on the network holds them for the network's worst case. One
that waits on a person holds them until the person comes back from lunch.
Gather the external inputs first, then open the transaction, then commit.

**`READ COMMITTED` is the default isolation level; `SERIALIZABLE` is opted into
per transaction where the domain needs it, with a retry**. The default must
be the one that does not surprise a developer with a failure mode they did
not write for. `READ COMMITTED` is that level: it never aborts a transaction
for a conflict the developer did not know could happen.

`SERIALIZABLE` is the correct choice where two concurrent transactions must
not both succeed. Examples are a balance that must not go negative, and a
seat that must not be sold twice. It is chosen deliberately there, with the
serialization-failure retry written alongside. A `SERIALIZABLE` transaction
without a retry fails a small fraction of the time for no reason a user can
see.

**A statement timeout is set on the runtime role**. A query that runs for
minutes holds a pooled connection for minutes. Under load the pool is the
resource that runs out first. The timeout is the ceiling a request is
permitted to spend in the database, set in configuration. A query that hits
it is a defect to fix rather than a limit to raise. This is the half of SD9's
bounded pool that bounds it in time as well as in count.
...
its first instance. SD3 keeps migration out of the serve process to avoid the
replica race. Two deploy steps overlapping, a retried pipeline or a person
and a pipeline, is the same race one layer up. One lock held for the run
means the second invocation waits and then finds nothing to do, which SD2's
convergence guarantees is safe.
...
([`026-blob-storage.md`](026-blob-storage.md) BS8). The argument is
compliance and security before it is engineering. A product can defend *we
do not hold data after its owner asked us not to* to a regulator, an auditor
...
financial record inside its statutory period, a message the other party still
holds, and an object whose history other rows reference. Where that is so,
the product says which tables and why in its **Conventions**, and three rules
hold on them:
...
- **A soft-deleted row is still personal data.** An erasure request reaches it
  as it reaches any other row. It does so in the redaction shape
  [`080-audit.md`](080-audit.md) AE7 defines, and under the
  [`082-data-subject-rights.md`](082-data-subject-rights.md). Soft delete
  defers deletion; it does not exempt from it.
- **Ordinary queries exclude it in the query text**, per SD1. The exclusion is
  a `WHERE deleted_at IS NULL` a reviewer can see, never a global filter a
  library applies invisibly.
...
invoice is gone and the record that it was voided on the fourth is not* is
the intended state. It is what makes hard delete safe as the default.

### SD13. The database is private to its service, and is tested as the real thing

**One service, one schema, one writer.** No other service reads this
service's tables, and no other service holds a credential to them.
Integration happens through the service's interface, never through a shared
database. The interface is its API per [`050-http.md`](050-http.md), or the
events of the [messaging standard](055-messaging.md). A table read by two
services is an interface with no contract, no version and no owner. The
...
SD1: the query text is the contract, and it is engine-specific in its
placeholders, `RETURNING`, `ON CONFLICT` and date arithmetic. A query proven
against a different dialect is a different query proven. A test suite that
cannot obtain a real engine does not run the data-access tests. It does not
run them against a substitute and report green.
