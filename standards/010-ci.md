...
its own jobs by local path, which matches nothing. `workflows` is therefore
held to this by review rather than by the checker — the reason its `release`
job was renamed to `version-release` by hand rather than by a failing gate.
