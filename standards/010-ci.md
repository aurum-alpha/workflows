...
**Where the fleet stands**, measured across all nine Node units with the job's
own configuration — `client-manager/client` and `gofast/client` are clean and
can block immediately; the seven Replit-derived repositories carry between 20
and 40 unused `dependencies` each and run `warn_only` until the dead-scaffold
sweep brings them down. No unit reports an `ERROR`, so the hard failure below
is armed rather than theoretical.
