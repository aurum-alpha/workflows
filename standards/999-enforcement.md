...
| AU2 | One access token crosses to the backend, and the backend verifies it | **the strongest gate here**: the token is a wire shape, so the corpus validates the required claims. The `typ`, `iss`, `aud` and `exp` checks are each a live case, and *verified rather than decoded* stays the black-box case of presenting a token signed with an untrusted key (proposed) | **review only** |
...
| — | The auth corpus decides the token and the client identity document | `job-contract-conformance` running [`../contracts/auth/corpus.json`](../contracts/auth/corpus.json) (proposed). **The two halves run in different places and the row states both.** The `validity` half is static: each case validates against `access-token.schema.json` or `me.schema.json` and reaches its stated verdict, with nothing running. The `behaviour` half needs a live stack with a provider behind it, so it runs where a repository has one and decides nothing where it does not | **review only** |
