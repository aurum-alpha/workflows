...
A provider takes the money; four of ten rules get nothing from it, the rest
only the far side of a call.
...
| BL10 | Nothing. | All of it (BL10): the audit event, the inventory entries, the retention treatment. |
...
Webhook schemes and catalog API coverage are the fastest-moving facts on this
page. Confirm both against the provider's reference before letting a row
decide.
...
| The catalog authored in the provider's dashboard, with the product reading it back | **BL1**: the catalog is a file in the repository, provisioned into the provider. |
| A plan changed in the provider's dashboard | **BL7**, and **BL6** turns it into a finding. |
| The provider's entitlements or features API consulted on the request path | **BL4**: the provider is never on the request path. |
...
| The reconciliation job writing ledger rows to make the two sides agree | **BL6**: a disagreement is a finding, never a silent write. |
