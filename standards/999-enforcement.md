...
| — | A rendered development stack equals the source it was rendered from | `check-devkit-drift`, rendering `devkit/` with the inputs the repository records in `deploy/devkit.json` and comparing bytes. Same shape as `check-lint-configs`, and the same finding: a shared file edited where it landed rather than where it is owned. **It compares against the catalog's current `devkit/`, and not against a ref the repository records.** A recorded ref lets a product keep a devkit that was fixed upstream, passing, until somebody bumps it — the drift this catches, wearing the costume of a version pin | gated¹ |
