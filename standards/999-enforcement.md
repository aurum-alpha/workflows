...
| AU9 | The ingress decides whether a login is needed, and the application decides what is allowed | `check-devkit-nginx` renders the example `deploy/public.json` and proves the locations it emits parse under the pinned nginx, and `check-devkit-drift` proves a product's rendered copy is what its own `deploy/public.json` says. Whether the application admits an anonymous principal on the same paths and runs its check on every route is the review question, and a catalog gate reading routes for it would be the PC4 violation. Each repository carries that assertion itself | **review only** |
