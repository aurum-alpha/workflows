...
| # | Rule | Enforced by | Status |
|---|---|---|---|
| BR1 | Every stateful backing service a service owns carries a recovery declaration | **schema-decided** for the declaration's shape under `job-contract-conformance`: sixteen `declarations` cases, with the four arithmetic relations as runner checks; that every store the configuration attaches appears in it is a review question until a checker compares the declaration to the declared configuration variables (030 SC3) | **review only** |
