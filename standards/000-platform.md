...
| Blob storage | S3 API profile as the storage protocol; one private bucket per service per environment; an object reference in the owning row, never a URL; presigned reads issued after the authorization check with a bounded lifetime; two-phase uploads the server verifies against storage and the bytes; scan before serve by declared audience; hard delete through the outbox and a periodic purge | [`026-blob-storage.md`](026-blob-storage.md) |
