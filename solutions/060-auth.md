...
below is a claim that some component satisfies a rule 060 states, and the date
...
edge. It confirms that the four verification items still name settings that
exist. A provider dropping `public` subject identifiers, or making pairwise
the only option, would be a finding against AU3. That belongs back in the
standard rather than on this page.
