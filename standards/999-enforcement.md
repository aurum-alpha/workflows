...
| 17 | Release is promotion, not production | `check-ci-conformance` D4, D5, REL | gated |
...
The job runs the schema-decided half of every vendored corpus, as each
corpus's `judged_by` map states it, with the runner checks the corpus names.
It proves `rbac/decisions.json` and `billing/entitlements.json` are fixtures
a runner can load, with the billing catalog valid against its schema. It
...
| LD9 | The local session cookie drops `__Host-` and `Secure` | `check-devkit-nginx` pins both oauth2-proxy cookie profiles on the catalog source. `check-devkit-drift` keeps a product's `dev/` on the local profile | **gated** |
...
| — | A rendered development stack equals the source it was rendered from | `check-devkit-drift`, rendering `devkit/` with the inputs the repository records in its root `devkit.json` and comparing bytes. Same shape as `check-lint-configs`, and the same finding: a shared file edited where it landed rather than where it is owned. **It compares against the catalog's current `devkit/`, and not against a ref the repository records.** A recorded ref lets a product keep a devkit that was fixed upstream, passing, until somebody bumps it — the drift this catches, wearing the costume of a version pin | gated¹ |
| — | The development stack's shared nginx configuration parses | `check-devkit-nginx`, rendering `devkit/` with throwaway inputs and running `nginx -t` under the image the fragment itself starts. **It runs in this repository alone**: the question is about the source, and a product re-validating a file it may not edit learns nothing | gated |
## Agent standard
...
| PC4 | Gates check the boundary, never the implementation | `job-contract-conformance` is the shape: it validates instances against schemas and loads fixtures, and reads no implementation's source; the behaviour half of every corpus runs in the product's own suite | **review only** |
...
| PC7 | A declaration a repository carries has one name and one home | every file named by an `x-repository-file` annotation validates against its schema under `job-contract-conformance`, in the repository under test; that every declaration a standard requires carries the annotation, and that the file sits at its stated home, are review questions until `check-contract-artifacts` (PC3) reads the table | **review only** |
...
| IP2 | The public id format table | **schema-decided** under `job-contract-conformance`: thirty-five `validity` cases against `primitives.schema.json`; the `canonical` part needs an implementation and is the product suite's | gated¹⁰ |
...
| # | Rule | Enforced by | Status |
|---|---|---|---|
| OC1 | Context propagates as W3C trace context, across every boundary | the `validity` half is **schema-decided** under `job-contract-conformance` (sixteen cases against `context.schema.json`); the `propagation` part needs a running service and is the product suite's; the envelope half is met by the CloudEvents tracing extension, gated by [`055-messaging.md`](055-messaging.md)'s schema | **review only** |
...
| AU2 | The credential is the provider's access token, and the backend verifies it | **live cases only, and deliberately so**: the shape is RFC 9068's and no contract here restates it, so nothing is schema-checkable. Four black-box cases carry the rule: a token signed with an untrusted key, one for another audience, one with no audience, and an ID token presented as a bearer. They are the auth corpus's `behaviour` part, which needs a stack and runs in each platform module's own suite, with `check-corpus-coverage` holding the set; the `validity` half has no token case | gated¹⁰ |
...
| AU4 | Users are created in the application, and the application creates the identity | two corpus behaviour cases: a second application reusing an identity, and revocation in one application leaving another unaffected (proposed). Which provisioning mode a repository runs is a **Conventions** declaration, and the rest governs who can change what, which no boundary reveals | **review only** |
| AU5 | Sessions end, and revocation does not wait for them to | live gates are available for the observable half: a session refusing service after its absolute cap, and logout ending the proxy session (proposed). Ending the provider session too is a product option, not the default. **Accepted gap:** back-channel logout is not implemented because oauth2-proxy does not support it; revocation rests on the five-minute token | **review only** |
| AU6 | An authenticated subject the application does not know is refused, and logged out | **a clean live behaviour case**: authenticate as an unknown subject, require 403 with the session ended. The `entitlements` block and `user.editable_fields` are schema-decided by `me.schema.json` under `job-contract-conformance`. That the server also enforces every permission and entitlement resists a checker and is the review question | gated¹⁰ |
| AU7 | The topology is one of three, and each states its cookie and CORS posture | `check-devkit-nginx` pins both oauth2-proxy cookie profiles. Live HTTPS `Set-Cookie` on a production host stays the review question (proposed) | **review only** |
| AU8 | A session is an authentication into exactly one tenant | **two live behaviour cases** (proposed): a session made on one tenant's entry presented to another tenant's resource is refused with `403` and audited, and a login response on a tenant host carries a `Set-Cookie` with no `Domain=`. A third, where a product admits a change of active grant: the change rebinds the session record and emits `auth.grant_activated`. That the binding is read from the session and not from the request resists a checker, because a gate reading routes for it would be the PC4 violation. It stays the review question: *how does a request's tenant get chosen, and by whom* | **review only** |
| AU9 | The gateway routes, and where a route goes is how it is authenticated | `check-devkit-nginx` renders a table carrying one route of every shape and proves the result parses under the pinned nginx; `check-devkit-drift` proves a product's rendered gateway is what its own `devkit.json` says. Whether a service authenticates what reaches it, and runs its check on every route, is the review question. A catalog gate reading routes for it would be the PC4 violation, so each repository carries that assertion itself | **review only** |
| AU10 | Each identity field has one master, and the application renders read-only what it does not master | — **resists a checker**: which field a provider masters is that provider's configuration, and a gate here would read a deployment rather than this repository. Two review questions: *does the application write `email_verified`, ever*, and *does any screen in it post a password*. Both are a grep in the application and neither is a boundary a corpus reaches | **review only** |
| — | The auth corpus decides the token and the client identity document | `job-contract-conformance` running [`../contracts/auth/corpus.json`](../contracts/auth/corpus.json). **The two halves run in different places and the row states both.** The `validity` half is static: each case validates against `me.schema.json` and reaches its stated verdict, with nothing running. The token has no validity case, because this repository describes no shape for it. The `behaviour` half needs a live stack with a provider behind it, so it runs in each platform module's own suite, and `check-corpus-coverage` holds the set | gated¹⁰ |
| — | One corpus judges every implementation of the active-grant resolver | each platform module's own tests running [`../contracts/tenancy/resolution.json`](../contracts/tenancy/resolution.json): a fixture, then how a session resolves, how a switch is answered and what the grants view says, each with the outcome AU8 and 092 TH1–TH3 fix. Static, and it needs no service, because the resolver is a pure function of its arguments | **review only** |
| — | Every name an implementation carries is the vocabulary's | each platform module's own test asserting the names in [`../contracts/platform/vocabulary.json`](../contracts/platform/vocabulary.json) are present under the language's casing and no name outside it is exported for the same concept. A name a module invents is a second answer to a question the file answers once | **review only** |
| — | Every corpus case a module answers for is named by a test | `check-corpus-coverage`, from `job-ci-conformance`: for each corpus a module lists in `contracts/ANSWERS`, every case id appears in a test file. A loop that runs whatever a corpus hands it names nothing, and a test suite that hand-picks ids executes a different set from the next module's; a table naming every id that the runner asserts is the corpus's whole set satisfies both. A repository with no `ANSWERS` has adopted no corpus and passes | gated¹ |
...
| # | Rule | Enforced by | Status |
|---|---|---|---|
| RB1 | Permissions are a closed set, declared in code | the decision corpus's rejection case carries the error behaviour; `job-contract-conformance` proves the fixture loads and each module's suite runs it. That the declaration is genuinely complete is a judgment about content | gated¹⁰ |
| RB2 | A permission is `resource.action` | **static and decidable**: every declared permission matches `^[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*$`, which also catches the three-segment form. Held over the decision corpus's declared permissions by `job-contract-conformance`, and by each module's suite over its own | gated¹⁰ |
| RB3 | A role is a named set of permissions, and every permission in it is real | three corpus rejection cases: an undeclared permission in a role, a tenant editing or deleting a system role, and a tenant role shadowing a system role's name | gated¹⁰ |
| RB4 | Code never branches on a role name | **one corpus case is a mechanical detector**: a subject holding a tenant-defined role that carries exactly the permissions of a code-declared one. An implementation gating on the name refuses a subject the permission model allows, and fails that case while passing every other. A grep for role-name comparisons is a weak second gate with real false positives: display and seeding are legitimate | gated¹⁰ |
| RB5 | A grant binds a subject to a role within a scope | **decided entirely by the decision corpus**: five containment cases including the two an implementation gets wrong, upward and sibling, each made in a stated active grant. The rejection case for a second grant, where the product declares one per subject, covers the **Conventions** half | gated¹⁰ |
| RB6 | Deny by default, one grant at a time, and no permission means "everything" | **decided entirely by the decision corpus**, plus a rejection case covering all four surfaces a wildcard could enter by. **One case is the detector for the model itself**: a subject holding two grants at one scope, acting in the one that does not carry the permission, where an additive implementation returns allowed. A pair of `permissionsFor` cases on that subject says the same thing as an enumeration | gated¹⁰ |
| RB7 | `check` is a pure function of its arguments | the corpus is only writable *because* of this rule, so passing it is the evidence. Purity itself resists a checker. A gate reading source for it would be the PC4 violation | gated¹⁰ |
| RB8 | A decision carries its reason | the reason's presence and shape are corpus-checked; whether it is *informative* is a judgment, like SC2's | gated¹⁰ |
| RB9 | A cached decision is keyed by everything the decision depends on | **two corpus cases, one per term a key loses**: the same permission checked in two scopes, allowed in the first and denied in the second; and the same permission at one scope under two active grants, denied in the first and allowed in the second. Each runs its first half as a `before`, so a cache missing scope fails the one and a cache missing the active grant fails the other | gated¹⁰ |
| RB10 | The scope of a request comes from the authenticated session, never from the request | **one live behaviour case**: a request carrying a tenant header or hostname for a tenant the session is not bound to, against a resource the subject would be allowed in that tenant, must be refused. An implementation reading scope from the request passes every single-tenant test and fails this one. Where the argument is actually read from resists a static checker honestly and is the review question | gated¹⁰ |
| — | One corpus judges every implementation of `check` | `job-contract-conformance` running [`../contracts/rbac/decisions.json`](../contracts/rbac/decisions.json). Static, and it needs no service: the fixture, an active grant per case, and the decision each case expects. **It decides the one thing nothing else can, that an implementation in one language and an implementation in another return the same decision**, because both read this file rather than each other | gated¹⁰ |
## Audit standard
...
| AE2 | One event shape, and actor is not target | **schema-decided** under `job-contract-conformance`: sixteen `validity` cases against `event.schema.json`; the `floor` and `redaction` parts need an implementation and are the product suite's | gated¹⁰ |
...
| SD3 | Migrations ship in their own image and run as a step before rollout | **live gate**: apply from empty, apply from the previous release's image, then replay every file against the migrated database bypassing the version record and require exit zero and an unchanged schema. It runs as a sibling of `job-image-starts` (proposed `job-migrate`). A schema check at start that reports on `/readyz` is a `job-image-starts` assertion against an old schema (proposed) | **review only** |
...
| # | Rule | Enforced by | Status |
|---|---|---|---|
| AM1 | A message is a CloudEvent, under this profile | **schema-decided** under `job-contract-conformance`: ten `validity` cases against `envelope.schema.json`. Past tense is the one half the schema cannot tell and stays a review question | gated¹⁰ |
...
| # | Rule | Enforced by | Status |
|---|---|---|---|
| JB1 | A job is a function from an input to an outcome | the declaration's shape is **schema-decided** under `job-contract-conformance` (twelve `declarations` cases, the name grammar included); the `keys` and `policies` parts need a runtime and are the product suite's; "knows nothing of its trigger" is a review question | **review only** |
...
| JB3 | Every job declares its class beside its code | **schema-decided**: twelve declaration cases, eight of them rejections each naming its rule; the worker's refusal to load an undeclared job is a `one_shot` corpus case; every `job-declaration.json` in the repository validates against the schema under `job-contract-conformance`, and a repository with none passes | **review only** |
...
| BS3 | The key is tenant, entity and object id, and carries nothing else | **schema-decided** under `job-contract-conformance`: twelve `keys` cases against `key.schema.json`, and the `references` cases' two key equalities as runner checks | gated¹⁰ |
...
| BS9 | Encryption at rest is on, and versioning follows the store's role | bucket configuration facts (default encryption, versioning state) readable by the same proposed `check-bucket-posture`; the version-id rule is a review question on the boundary module | **review only** |
...
| # | Rule | Enforced by | Status |
|---|---|---|---|
| DS1 | The relational store is the system of record, and a document store is admitted by declaration | **schema-decided** for the declaration under `job-contract-conformance`: sixteen `admissions` cases against `admission.schema.json`, with the profile, schema-version, credential and index judgments as runner checks; that a store exists without an admission is a credential fact (a document-store connection variable in a service's configuration with no admission beside it) checkable once credentials are declared per image (035 WK8); whether the stated reason is true is a review question; every `document-store-admission.json` in the repository validates against the schema under `job-contract-conformance`, and a repository with none passes | **review only** |
...
| DS5 | Every document carries the envelope | **schema-decided** under `job-contract-conformance`: ten `envelopes` cases against `document-envelope.schema.json`, with the scoped-needs-tenant and size-ceiling judgments as runner checks against the case's collection; one case is recorded as passing because a 24-hex engine identifier is a well-formed nanoid and no schema can tell them apart | gated¹⁰ |
...
| # | Rule | Enforced by | Status |
|---|---|---|---|
| BR1 | Every stateful backing service a service owns carries a recovery declaration | **schema-decided** for the declaration's shape under `job-contract-conformance`: sixteen `declarations` cases, with the four arithmetic relations as runner checks; that every store the configuration attaches appears in it is a review question until a checker compares the declaration to the declared configuration variables (030 SC3); every `recovery-declaration.json` in the repository validates against the schema under `job-contract-conformance`, and a repository with none passes | **review only** |
...
| SE2 | Every secret is declared, and the declaration is the source of truth | **schema-decided** under `job-contract-conformance`: thirteen `declarations` cases, with unique names and one credential per backing service as runner checks; every `secret-declaration.json` in the repository validates against the schema in the same job, and a repository with none passes; the environment-vs-declaration diff is a live check in `job-image-starts` (proposed `check-secret-declaration`); that no process opens the file is a call-graph fact (PC4) and stays a review question | **review only** |
...
| SE4 | No secret value enters the repository, at any point in its history | **the gate is `job-secret-scan`**: gitleaks over every commit the event judges, and `check-secret-locations` over every tracked file it judges (a tracked `.env` path, a value on a secret name in `.env.example`, a Dockerfile `ENV` or `ARG`, a compose credential to a host the project does not start, a repository-side allow-list). The job proves both halves against the fourteen `forbidden_locations` cases before it judges, the low-entropy detector included. A pull request is blocked on what it introduces; the standing state blocks under `fail_on_existing`. Host push protection is an organisation setting an owner enables. Review-only until repositories call the job | **review only** |
| SE5 | A secret is never in an image, a log line, a URL or an error body, and it is recognised by declaration | the Dockerfile half is **a grep with no false positives** (`ENV`/`ARG` naming a secret-grammar variable), `check-secret-locations` under `job-secret-scan`, three `forbidden_locations` cases; the log half is the `redaction` corpus against a running emitter, which is the product suite's, **one case a verified detector**: a passphrase-shaped secret a shape-filtering redactor misses. The URL and error-body halves are HA3's problem+json validation plus review | **review only** |
...
| SE9 | Local development and the pipeline use their own secrets, and `.env.example` is the contract | the `.env.example`-matches-declaration diff is a static check beside SE2's; the compose admission is two `forbidden_locations` cases, decided by `check-secret-locations` under `job-secret-scan`: a credential to a host the compose project does not start is the finding; that a development credential grants nothing outside the developer's machine is review | **review only** |
...
| FF8 | Every evaluation is observable on the span, and none is a log line at volume | the attribute names are checkable against the semantic conventions where the propagation corpus runs live, in the product suite; the no-line-per-evaluation rule is a review question on every diff that logs | **review only** |
...
| NF3 | The record is the source of truth for *did we tell them*, and it is the service's | **schema-decided** under `job-contract-conformance`: the `records` cases, each against the schema it names, `notification.schema.json` among them, including the address, body, missing-provider, missing-reason and security-by-preference rejections. Log fields are checkable against the OC4 block; the alert is platform configuration | gated¹⁰ |
...
| # | Rule | Enforced by | Status |
|---|---|---|---|
| DR1 | The data inventory is the precondition, and it is a declaration | **schema-decided** for the declaration (eight `inventory` cases) and **catalog-decided** for completeness: the `coverage` corpus part enumerates tables the way 025 SD6's gate does and names the undeclared table, the unknown column, the dangling `via`, the duplicate membership entry and the column with no tombstone (proposed `check-data-inventory`); every `data-inventory.json` in the repository validates against the schema under `job-contract-conformance`, and a repository with none passes | **review only** |
| DR2 | Both rights are request resources under one status machine | the resource is **schema-decided** under `job-contract-conformance` (seven `requests` cases: each status requires and forbids its fields); the `transitions` part decides the open-request `200`, the cancellation rules and the download route in the product suite; that the deadline alert is wired is platform configuration read from the request table | **review only** |
...
| SB5 | Unauthenticated and authentication routes are rate limited, and the refusal is HA7's | the refusal shape is 050 HA7's corpus case under a live harness; that a limit exists on an unauthenticated route is a live test that sends more than the floor (proposed); the error-report intake's 1/s bound is a named class of that floor (090 WC5); whether the floors were chosen rather than defaulted is a review question | **review only** |
