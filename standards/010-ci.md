...
- *Packages reachable only from dead files.* knip prunes unreachable files
  ...
  as unused. That is true, but it is a **dead-file** finding wearing a
  dependency's name, and it is why the gate cannot block yet: roughly twelve
  `@radix-ui/*` packages sit behind unused shadcn components in wardley-mapper
  alone, out of 71 unused files there.
...
**Where the fleet stands**, measured across all ten Node units with the job's own
configuration:

| unit | unused `dependencies` | unused `devDependencies` |
|---|---|---|
| `gofast/client` | 0 | 0 |
| `client-manager/client` | 0 | 1 |
| event-manager | 17 | 8 |
| jewelry-factory | 23 | 2 |
| hiring-tracker | 24 | 5 |
| credit-watch | 32 | 1 |
| flight-watch | 35 | 0 |
| expense-splitter | 36 | 3 |
| wardley-mapper | 40 | 2 |

`gofast/client` is the only unit that measures clean, and the only one that
blocks. Everything else runs `warn_only` until the dead-scaffold sweep brings
the counts down. No unit reports an `ERROR`, so the hard failure below is armed
