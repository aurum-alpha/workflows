...
actually holds. Artifacts: [`contracts/billing/`](../contracts/billing/).

This document defines **what a tenant bought, and how the product knows**, as
an interface specification: a catalog, a subscription that is a projection of
an append-only ledger, four kinds of entitlement, a pure function that derives
them at any instant, a fixed place in the request for the check to run, one
adapter to the provider that takes the money, and a corpus of decision cases
any implementation in any language must reproduce. It sits between
[`060-auth.md`](060-auth.md), which produces the tenant, and [`070-rbac.md`](070-rbac.md),
which decides what a person may do inside it, and it feeds
[`080-audit.md`](080-audit.md) and [`082-data-subject-rights.md`](082-data-subject-rights.md),
which record and export what it did.

## Why this exists

Three questions in a product each produce a yes or a no, which is the whole
reason they get confused: *is this code path live*, *did this tenant's plan buy
this*, and *may this person do this here*. They differ on every axis that
matters, and a codebase that answers the second with the machinery of the
first or the third acquires a defect that no test written against one tenant
will show. **The test is to ask who flips it.** An engineer flips a flag. A
payment flips an entitlement. A tenant administrator flips a permission, for
one person, by granting a role.
...
|---|---|---|---|
| The question | Is this code path live right now? | Did this tenant's plan buy this? | May this person do this here? |
...
One function can be under all three at once, and the handler asks them in a
fixed order, each answering a different question, so that a denial always
names which one said no (BL4).

The second thing this document settles is **where the truth is**, because
billing is the one capability where *the record in our database is the truth*
is half right. The product's subscription record and ledger are the truth for
**entitlements**: what this tenant may do now, and what it could do on any
past date. The billing provider is the truth for **money**: what was charged,
...
identity tier, and prices are read from one catalog and never typed three
times.
...
product sells is declared in one versioned file in the repository, validated
against [`catalog.schema.json`](../contracts/billing/catalog.schema.json) in
CI and built into the image.** The billing provider's products and prices are
created *from* that file by a job, and the provider's ids are stored against
the catalog's plan, interval and currency. A price that exists only in a
vendor's console is a price no pull request reviewed, and a price typed into a
page is a second source that disagrees with the first the week it matters.

The catalog carries four closed sets and then the plans:

| Section | Carries |
|---|---|
| `capabilities` | Every capability the product can gate, each with an id and a description. A capability may be in no plan; it is still declared here, because the code that checks it exists whether or not anything sells it yet. |
...
The file also states the policies BL4 reads: the past-due grace period in
days, and the **suspended set** — the capabilities that remain when a plan has
stopped contributing. Both are policy, and a policy that lives in code is a
policy nobody reviews as a diff.
...
the catalog no longer carries, and never deletes or edits a live price — a
price is immutable once charged, so a changed amount is a new price object and
a new catalog version — and it writes the id mapping into the service's own
database (025 SD13), where the adapter of BL5 reads it.

**Prices are grandfathered; entitlements are current.** A tenant stays on the
price it signed at until it changes plan, because the provider's subscription
references the price object it was sold and the ledger row records the catalog
version it was priced under (BL2). What the plan *entitles* is always the
current catalog's definition, because the code serving the tenant is the
current code and can honour only the capabilities it has; a per-version
entitlement set would make the function of BL4 hold every catalog ever
shipped.

A public surface that shows prices renders them from the catalog — as a
declared build source, or from a public, cacheable, unauthenticated read the
product's OpenAPI document names ([`050-http.md`](050-http.md) HA2) — and never
from content of its own.
...
later than its recorded time is the subscription's pending change — a
...
and its successor is its explanation. That is what bounds the projection to
one pending change without a second table, and it is what lets the function
of BL4 be asked about any instant, past or future.
...
is one of the tables a product names under [`025-structured-data.md`](025-structured-data.md)
SD12's retention exception, and its retention is stated there.
...
| **quota** | a ceiling on what may **exist at once** | Compared against the domain's own count when something is created. | `403 entitlement-required`, at creation only. |
| **allowance** | a ceiling on what may be **consumed per billing period** | Compared against a usage record the domain writes and that resets on the period boundary. | `403 entitlement-required`, or overage where the plan sells it. |
...
states. An allowance is consumed: it resets each period, it needs a usage
record the domain writes, and it is the only kind that can connect to metered
billing. A product with no metered feature has no allowances, and the kind
stays empty.

**Nothing is per user.** What one person may do inside a tenant is a
...
not a permission — edited outside the grant path, invisible to
`permissionsFor`, and the failure RB4 describes arriving with a price tag.
**Terms** — a support tier, an SLA — appear in the catalog so a pricing page
can render them, and no code checks them.
...
reader or by a linter, and a refusal can name what was missing without
saying which kind it was.

Three shapes are refused by this rule, and each is the same mistake in a
different table. **A role named after a plan** — an upgrade would mean
reassigning roles to every user in the tenant; roles say who a person is
inside the tenant, the plan says what the tenant bought. **A permission that
encodes a plan** — the permission set is what the product *can* do, the
catalog is what a tenant *bought*. **A flag kept on because a customer pays
for it** — a flag has a bounded life and an engineer flips it; this one would
never expire and money flips it, so it is an entitlement wearing a flag's
name, and 038 no longer has a kind for it to be.
...
instant produce the same set every time. Nothing ambient — not the clock, not
a session, not the provider — enters it. Purity is what makes the corpus of
this document writable and a cache safe, and it is why BL2 requires effective
...
every quota and allowance is zero, so nothing new is created, and every
setting keeps the plan's value, because a setting such as how long history is
kept is exactly what governs a tenant that has stopped paying. Reading what
exists and reaching the billing settings are ordinary routes under
permissions and were never capabilities; a product that has put either behind
one names it in the suspended set.
...
status, because it is a deliberate act by a named administrator with a stated
expiry, and a function that silently discarded such an act would make the
administrator's screen untrue. An administrator who wants it gone revokes it.
Where an override and the plan both speak, the override wins.
...
dressed as a helpful one, and the plan is information the tenant's
administrators decide who sees.
...
`subscription.schema.json`: `entitlement` — the kind, the id, and for a quota
or allowance the limit — and `plan`, the tenant's current plan. That is what a
client needs to render *this needs the business plan* without a second
request, and it is a statement about the tenant, not about the person, so it
crosses the boundary where RB8's reason does not.
...
`403` says exactly what is true — the request is understood and refused — and
HA3 already makes `type`, not the status code, the thing a client branches on.

#### The cache, the client, and the provider

**A cached set is keyed by the tenant, the catalog version and the
subscription's last ledger row, and it carries the instant it stops being
valid.** The first three are every input of the function that a row or a
deploy can change, so every ledger row and every catalog release invalidates
by construction, as RB9 requires of a permission cache. The fourth exists
because this function also depends on time: a grace period ends, a period
ends, a trial ends, an override expires, a pending row takes effect, and no
new row announces any of them. So the set states `valid_until`, the earliest
such boundary, and the cache honours it. A TTL is the backstop, not the
mechanism.
...
imports no vendor SDK; and no server image holds a provider credential that
the pool and jobs images do not need more.** This is the shape
...
hosted checkout or by its tokenised fields rendered in the browser, and the
product receives a token or a redirect, never a number. That is the difference
between a product that is in scope for card-handling compliance and one that
is not, and it is not a choice a repository makes. The provider's browser-side
key, where tokenised fields need one, is public by the provider's design and
reaches the page through the runtime configuration of
[`090-web-client.md`](090-web-client.md) WC2, never compiled into the bundle.

A **promotion code** is non-personal context, so a public surface may carry
one in the hand-off to sign-up; its validity is decided here, at checkout, and
nowhere earlier. Invoices and the billing portal are read through the adapter
and served by the product.
...
[`055-messaging.md`](055-messaging.md) AM8: verified over the raw body inside
the adapter, re-enveloped with the provider's event id, recorded durably,
acknowledged, and only then processed.** The consumer maps each
`ProviderEvent` to a ledger row with actor `provider_webhook` and the provider
event id on the row, so that a redelivered event is a repeat under AM3 and
never a second row. A period renewal updates the projection's period fields
and writes no row (BL2).
...
the provider's record — plan and price, status, period, whether cancellation
is scheduled — and **a disagreement is a finding, never a silent write.** The
job records what it found on its run record (JB5) and alerts; it does not
write a ledger row to make the two agree, because which side is wrong is a
judgment. A person decides, and the correction is a ledger row by an
`internal_admin` with a reason. The provider is polled by this job and by
nothing else.

### BL7. Plan changes are made in the product, under stated policies

**A plan changes through the product's own routes, or through an internal tool
calling those routes over [`050-http.md`](050-http.md) with a
[`070-rbac.md`](070-rbac.md) permission, and never in the provider's
dashboard.** A change made at the provider moves the money truth and not the
entitlement truth; BL6's job turns it into a finding.
...
**Above a new quota, nothing is deleted and nothing new is created.** A
tenant that downgrades to three seats while holding seven keeps seven; the
eighth is refused until the count is under the ceiling. Deleting a customer's
data to fit a plan is a destructive act nobody asked for; refusing creation is
a state the interface can explain.

A comp, a discount or a manual extension is an override or a plan change with
the staff member as actor and the reason on the row, audited under BL10. Staff
never act through a customer's session.
...
past its `trial_end` it writes `trial_converted` where the provider has taken
payment — usually the provider's webhook has already written it, and the job
finds nothing to do — and `trial_expired` otherwise, which is `cancelled` with
no plan from that instant. The warnings before the end are
[`058-notifications.md`](058-notifications.md) notifications in a declared
category; the fact of the transition reaches a CRM or anything else through
the outbox (055 AM4) like every other ledger row.
...
on a paid plan; a product may have either or both and states which in its
...
**Sign-up runs in the product.** A public surface offers the plans and hands the
visitor across with nothing but non-personal context — the plan and a campaign
source as query parameters, a promotion code at most — and never renders a form
that collects a name or an email for sign-up.

In the product, one transaction writes the tenant, its first user with the
administrative grant, the subscription projection with its first ledger row —
`trial_started` where the plan carries a trial, `upgraded` from no plan
otherwise — the outbox events that announce the tenant, and the audit events.
The identity is created at the identity provider by the application, per
[`060-auth.md`](060-auth.md) AU4, through the idempotent `ensureIdentity`; the
provider customer is created through BL5's adapter with the tenant's public id
as the idempotency key, and the projection's provider ids are absent until the
provider confirms them. Both calls cross a boundary on a key the product
chose, which is what lets the local transaction be retried without a second
tenant. A tenant with no subscription is one the entitlement function cannot
answer for, and a subscription with no tenant is a row nothing owns.
...
tenant's plan and why* is the question that gets asked. Audit is a consequence
of the ledger, never its source: the ledger row is the domain fact and the
audit event is the record that it happened.
...
erased is [`028-backup-and-recovery.md`](028-backup-and-recovery.md)'s and 082's
business — the ledger under a `retain` treatment with a named basis and an
expiry (DR5), everything else under the product's stated retention — and it
is stated to the tenant up front.
...
- **`subscription.schema.json`** — BL2's projection, ledger row and override;
  ...
  members.
- **`entitlements.json`** — **the corpus that matters**: a catalog,
  subscriptions in every status, a ledger, overrides including an expired
  one, and a fixed `now`; then checks with expected decisions and stimuli that
  must be refused. An implementation in any language loads the fixture,
  evaluates every case, and either reproduces each decision or names the one
  it failed. No running service, no provider, no network.
...
- **BL4's derivation, BL2's effective times and BL3's kinds are decided by the
  corpus.** The cases worth having are the ones nobody tests by hand: a
  past-due tenant one day inside its grace period and one day outside it; an
  override that expired yesterday; a cancelled tenant on the last day of its
  period and the first day after; a pending downgrade not yet applied; a quota
  the tenant already exceeds after a downgrade; a capability present in the
  catalog and absent from every plan; the same tenant asked about a date
  before its last upgrade; a superseded cancellation. Each is a case, and
  several are detectors for a specific shortcut.
- **BL1 and BL3's shapes are schema-decided**: a dotted capability id, a float
  price, an unknown ledger kind, a second pending change and a per-user
  entitlement are each refused by a schema before any code runs.
- **BL4's cache rule is caught by a corpus case, not a static checker**: the
  same capability asked inside and then outside a grace period with no ledger
  row between. A cache keyed without `valid_until` returns the first answer
  twice.
...
- **This document owns the entitlement check, and 038 gives up its
  `entitlement` flag kind** (2026-09-08): a flag evaluation returns a value and
  a closed reason enumeration; it cannot say which plan refused, cannot carry a
  numeric limit with a period, and cannot be asked about a past date. The
  question *did this tenant buy this* therefore needs its own function, and
  once it has one, a flag answering the same question is a second read path
  for one fact — one source of truth violated at the read side, with the two
  paths free to disagree. Money flips an entitlement and an engineer flips a
  flag, and that difference is the boundary.
- **Four kinds, closed** (2026-09-08): capability, quota, allowance and
  setting are distinguished by what the code does with the value, which is
  the only distinction an implementation can be held to. A fifth kind would
  be one of these with a different name or a term nobody checks.
...
- **The catalog is a file and the provider is provisioned from it**
  ...
  alternative, the provider's console as the source, puts the one artifact the
  pricing page, the product and the entitlement function all read behind a
  login and outside review.
- **A projection over an append-only ledger with effective times**
  ...
  entitled to last month, and a scheduled change needs a place to live.
  Effective times give both with one mechanism, and a row recorded while
  another is pending superseding it is what bounds the projection to one
...
- **Prices grandfathered, entitlements current** (2026-09-08): the provider
  ...
  every catalog ever shipped would make the function's inputs unbounded and
  its behaviour depend on which version a tenant happened to sign under.
- **Suspension is a policy the catalog states, and an override survives it**
  ...
  that belong in a reviewed file, not in a handler. An override is a named
  administrator's deliberate act with an expiry, and a function that
  discarded it on a status change would make the administrator's screen
  untrue; revocation is one call away.
- **`403` with a distinct problem type, not `402`, and permission before
  ...
  reveals nothing about the plan, and an upgrade prompt shown to someone who
...
- **The provider is never on the request path** (2026-09-08): the ledger is
  ...
  and it couples the product's availability to a vendor's. Reconciliation is
...
- **`valid_until` in the entitlement set** (2026-09-08): a permission cache is
  invalidated by writes alone; this function also changes with time, and a
  cache keyed only on the ledger's last row would keep serving a plan through
  the end of its grace period. Stating the next boundary makes the cache
  correct rather than merely short-lived.

## Out of scope, deliberately

- **The pricing page, the sign-up hand-off and lead capture.** The [web
  estate standard](091-web-estate.md)'s; this document
  binds what the page renders from and where the hand-off lands.
- **Tax, invoicing accounting, revenue recognition, dunning cadence.** The
  provider's, behind the adapter. This document fixes what the product does
  when told a payment failed and when it recovered.
