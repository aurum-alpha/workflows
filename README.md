...
| `016` | LD | [`016-local-development.md`](standards/016-local-development.md) | Local development: a process reads its listen port from the environment with a default in code, a container binds defaults so a development compose file sets no port, host bindings come from one aligned block of twenty with offset zero the thing a person opens, and the development image is `Dockerfile.dev` installing from the lockfile with the dependency tree in a volume the container populates | review only |
