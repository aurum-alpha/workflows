...
**Whether an id is well formed is a separate question, and it is not gated
yet.** The grammar is
`<purpose>-<language>[-<framework>][-<product>]-<capability>[-<tool>]`, and it
earns its keep outside CI: these ids are what a human reads in tooling that
spans repositories, so an id that lies about what it runs costs more there than
here. Checking it against the declaration of the job actually called is what
the declarations make possible.
