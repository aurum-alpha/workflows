...
| # | Rule | Enforced by | Status |
|---|---|---|---|
| RB1 | Permissions are a closed set, declared in code | corpus rejection case for the error behaviour, named by `job-contract-conformance`. That the declaration is genuinely complete is a judgment about content | gated¹⁰ |
| RB2 | A permission is `resource.action` | **static and decidable**: every declared permission matches `^[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*$`, which also catches the three-segment form. Held by the decision corpus under `job-contract-conformance` | gated¹⁰ |
...
| SE2 | Every secret is declared, and the declaration is the source of truth | **schema-decided**: thirteen `declarations` corpus cases under `job-contract-conformance`; `secret-declaration.json` at its fixed path validating against the schema is a file check in the same job (proposed); the environment-vs-declaration diff is a live check in `job-image-starts` (proposed `check-secret-declaration`); that no process opens the file is a call-graph fact (PC4) and stays a review question | **review only** |
