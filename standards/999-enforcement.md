...
| AU9 | The ingress decides whether a login is needed, and the application decides what is allowed | `check-devkit-nginx` proves the rendered ingress parses with the public locations in it, and `check-devkit-drift` proves the rendered copy is what `deploy/public.json` says. Whether the application admits an anonymous principal on the same paths and runs its check on every route is the review question, and a catalog gate reading routes for it would be the PC4 violation. Each repository carries that assertion itself | **review only** |
