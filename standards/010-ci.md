...
15. **The repo is versioned, not the artifact.** Artifacts built from one
    ...
    **Which artifacts share an image is decided by three things**: the
    ...
    those — because a server and a worker differ in closure, configuration
    and scaling, the migrate step differs in credential
    ([`025-structured-data.md`](025-structured-data.md) SD3), and a bundle
    differs in all three. A repository holding several services builds a
    full set of images per service, because their credentials differ. Two
    stacks that ship together are two images of one run bound by its version,
    not one image bound by its contents. How workers apply the three criteria
    is [`035-workers.md`](035-workers.md) WK2's.
...
19. **The catalog is where best practice lives; a repo-local job is a gap in
    ...
    base that carries no source of ours, is work one product does and no shared
    job models. It is often false, and then the fix is a catalog job, not a
    local one.
    ...
    **The job id is what separates the two, and it is not advisory.** A job id
    ...
    and it runs on the tree rather than on a list of exceptions — the catalog
    holds no register of which repositories are permitted which local jobs, and
    it held one until it was noticed that four of its thirteen entries had gone
    stale into permanent, silent waivers.
    ...
    until what is left in each repository is genuinely its own.
...
- A stack's gates depend on **that stack's** build and nothing else. A failing
  ...
  the other half.
- The convergence point is the first job that needs every stack's build: the
  image jobs, one per image, and the start check that runs the images
...
That is a deliberate reversal, and the argument it reverses is worth stating
because it sounds right. Pinning gave each repository a stable set of rules
that changed only when it chose, which made every adoption a canary and kept
...
standard, and a repository that can sit on an older catalog can decline a rule
indefinitely without ever saying so. Meanwhile the loop of *change the catalog,
bump eleven pins, discover the bug, fix the catalog, bump eleven pins again*
was itself the dominant source of breakage — a canary that must be carried to
...
the first repository to call it fail rule CAT, told by a checker reading a tree
from before the job existed. A data change (`dependency-versions.json`, wave A)
failed a repository for dropping packages the portfolio had already retired.
And a conformance run went green on a checker its own change had disabled,
because the declarations that checker needed were not yet on the branch it
read — an instrument reporting its own blindness as compliance.

With callers on `@main` the workflow and its checkers come from the same
commit. The two clones the job used to make are one clone, and a checker added
and wired in the same pull request is coherent by construction rather than by a
...
resolved to when the run was queued, and a re-run replays exactly those. So a
...
progress on every ref but the default branch, so restarting an old run re-enters
that group and cancels the live one. The result is a cancelled run on the commit
that mattered and a completed run on a commit nobody will merge. Before
re-running anything, check that its head is still the branch's head.

**A pull request that has fallen behind is not owed its old verdict.** Bring the
branch up to date and let CI run again against the standard as it is now. If the
answer changed, the answer changed — that is the standard doing its job, and the
work is to converge, never to reach for the ref that used to pass. A repository
whose only route to green is an older catalog is a repository that has stopped
complying and not yet been told.

The failure this rules out is the one every pinning scheme eventually produces:
a green check that asserts conformance to a standard nobody runs any more.
...
job hands out `stamp` and puts it into no artifact, because what a repo shares
with the rest of the portfolio is the *decision* — is this a release, and what
is a mid-stream build called — and only the rendering is per-ecosystem. A JS
repo that later grows a provenance file (Principle 13's "generated file at a
fixed path in `dist/`") reads that string rather than deriving a second answer
to a question already answered. The alternative was to leave the JS build
system out of the table, which is how a repo ends up writing
...
same reason. **That reason has since expired.** It cost a SHA pin, and so two
commits for every edit; first-party references are `@main` now and it costs
neither. The duplicate outlived its justification, which is worth its own
...
a catalog job and should not become one: it would have exactly one caller, and
the catalog is for capabilities the portfolio shares. The block is copied whole
rather than trimmed to the half a non-compiling repo uses, because a partial
copy is a copy the checker cannot compare.
...
fourth. Callers pass `stamp`, not `version`: `version` is what the last
release was called, and a mid-stream image labelled with it claims every merge
is that release. Left unset the label reads `dev`, which is what the shared
block itself returns for a repo with no version file and what a repo whose
only provenance is its commit SHA should say.
...
gives one answer to, and every one of them now resolves to the direct-invocation
job:
...
`job-node-typecheck` went first: client-manager was its last caller and now calls
...
Two jobs deleted alongside them for the same reason and with no callers at all:
`job-docker-image` and `job-image-docker`, near-anagrams of one another, both
declaring `capability=image role=publish`. Two files owning one capability is
the ambiguity every rule in this section exists to prevent, and neither had ever
been called.
...
loudest way available, that a capability it depends on moved — and it converges
on the job that owns that capability. Nothing is stranded by this: a pull request
runs the workflow file from *its own head*, so the branch carrying the fix builds
and reports normally even while `main` is red. Backward compatibility is not
offered, and a deprecation window would only mean the old answer survives long
enough for someone to write a third.
...
on the React compile. That is deliberate: the two artifacts ship in one run at
one version (Principle 15), so a failed React compile means nothing ships.
...
label from its computed tag list, and with no `tags` input the list is the
...
label, which is also why the version has to reach it as an input — see
...
fix is live on their next run. This paragraph used to describe the opposite —
callers reached a composite by SHA, SHAs moved through tags, tags came from
`.version`, and `check-standard-ref` had to be taught which paths were consumed
so a composite could not be fixed in a way that never reached the caller
needing the fix. None of that machinery exists now, and the failure it guarded
...
point every gate hangs off. D4 and D5 fired on `image-prune`, which contained
...
from the first, which is why the fix below removes the reading rather than
adding a third pattern.

**What a job is, is read off the workflow it calls.** Every catalog job
declares its capability, language and role on its first line, and the checker
...
and D5 altogether, while a PHP repo's `package`, which lays a vendor tree into
a dist directory, was failed as an ungated publish. An id is evidence about a
job the standard names; the `uses:` line is the job.
...
— which is what the declarations made possible, and the reason this could not
be gated before them.
...
the first two segments mandatory. That was wrong in a way worth recording: it
would have failed `osv-scan`, `coverage-upload`, `ci-conformance` and
`version-gate` — ids that are correct, and that no prefix improves.
...
calls the same catalog workflow twice, no caller may hold the bare capability
— whichever kept it would read as *the* one and its peer as a variant, when
they are peers. event-manager calls `job-image-cached` twice, for the builder
image and the production base, and `build-image-cached` / `base-image-cached`
say which is which at a glance.

**What the rule does not reach is the catalog's own jobs.** It resolves a job
through an `aurum-alpha/workflows/...@main` reference, and the catalog calls
...
pull request changes them*, not the rules already sitting on `main`. A
self-reference at `@main` would invert exactly that — a pull request editing a
rule would be judged by the version it is replacing, no rule change could
exercise itself before landing, and a rule that breaks the fleet would be
...
than by the checker — the reason its `release` job was renamed to
