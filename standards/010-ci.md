...
| `client-manager/client` | 0 + 1 | 0 + 1 | warn_only |
| event-manager | 17 + 8 | 17 + 8 | warn_only |

Seven of nine block. The two that do not have named blockers rather than open
windows: `client-manager/client` needs its `lint` script to stop invoking an
undeclared `eslint`, which is the GitHub Packages auth work, and reaching zero
by deleting that script would be silencing a gate the standard wants to exist.
event-manager's remaining findings are a jQuery/Bootstrap/grunt legacy stack,
not scaffold, and are their own sweep.
