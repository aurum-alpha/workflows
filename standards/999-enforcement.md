...
| FF9 | Values live in the provider, existence in the declaration; the provider is attached by configuration, one of **two** admitted shapes stated in Conventions — an off-the-shelf flag service, or a thin adapter over the product's own tables for `entitlement` flags alone; flag values never ship in the release, and the file-provider shape is refused with them; no process holds another service's provider credential | the Conventions declaration is a grep; the credential half is the per-image credential fact WK8's row waits on. That a declared shape is one of the two, and that no bespoke flag machinery has grown beside it, is a review question — the refused shape is defined by what a value can do rather than by a file a checker could name | **review only** |
